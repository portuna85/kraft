...
| 관리자 | `ROLE_ADMIN`만 통과 | 신고 처리·정지 회원 화면(`/admin/**`)과 그 API(`/api/v1/admin/**`) |
...
| POST | `/api/v1/admin/users/{id}/suspension/lift` | 관리자 | — | 204 (정지 해제) |
...
없으므로 **배치가 멈춰서 정지가 안 풀리는 일도 없습니다.**

지금 정지 중인 회원은 `/admin/users`에서 기한·사유와 함께 보고, 기간 전에 풀 수도 있습니다 —
정지는 사람의 판단이라 되돌릴 길이 필요합니다. 잘못 건 정지를 일주일 동안 그대로 두는 것은
그 사람에게 실제 손해입니다. 만료된 정지는 목록에 나오지 않습니다(이미 풀린 것과 같습니다).
해제해도 사유는 지우지 않습니다 — 무슨 일이 있었는지는 남아야 합니다.

처리한 신고도 행으로 남습니다 — 같은 대상이 반복해서 신고되는지, 관리자가 무엇을 언제
지웠는지가 남아야 나중에 설명할 수 있습니다.
