...
| 브랜드 | Kraft | 항상 `/` |
| 커뮤니티 | 전체 게시글 | 항상 `/` |
| 커뮤니티 | 글쓰기 | 로그인 시 `/posts/save` |
| 계정 | 로그인 | 비로그인 시 `/login` |
| 계정 | 회원가입 | 비로그인 시 `/signup` |
| 계정 | 비밀번호 변경 | 로그인 시 `/users/me/password` |
| 계정 | 로그아웃 | 로그인 시 기존 CSRF 폼 제출 |

로그인 상태에 이메일을 표시할 때는 계정 메뉴 내부에서 보여준다. 작성자 이름은 서버가 내려준 `author`를 그대로 사용한다. 프로필 페이지나 닉네임 수정 링크는 추가하지 않는다.
...
| --- | --- | --- | --- |
| 320~575px | 단일 열 | 헤더 메뉴 버튼으로 펼침 | 제목 위주 행, 좌우 16px, 패널 안쪽 16px |
| 576~767px | 단일 열 | 헤더 메뉴 버튼으로 펼침 | 제목+메타 행 유지, 폼 버튼은 공간에 따라 가로 정렬 |
| 768~991px | 본문 + 아래쪽 이용 안내 | 헤더 메뉴 버튼으로 펼침 | 열 정렬 목록, 좌우 24px, 패널 안쪽 24px |
| 992~1199px | 왼쪽 탐색 192px + 본문 | 탐색 상시 표시 | 열 간격 24px, 이용 안내는 본문 아래 |
| 1200px 이상 | 탐색 192px + 가변 본문 + 안내 248px | 탐색 상시 표시 | 열 간격 24px, 컨테이너 최대 1280px |

1280px는 컨테이너의 내부 콘텐츠 최대 폭이며 바깥 여백은 별도다. 폭 1200px에서는 바깥 여백 48px를 제외한 1152px 안에 `192 + 24 + 664 + 24 + 248` 배치가 들어간다. Grid의 본문 열은 `minmax(0, 1fr)`, 내부 요소는 `min-width: 0`을 기준으로 한다.
