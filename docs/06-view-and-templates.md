...
### 네비게이션 바 정책 조정 + 로그아웃 목적지 예외 (2026-09-10)

`/login`·`/logout`을 고친 직후 사용자가 두 가지를 추가로 요청했다: **(1)** "글 등록" 버튼은
로그인했을 때만 보이게 할 것, **(2)** 게시글 등록 화면(`/posts/save`)에서 로그아웃하는 경우는
(다른 화면과 달리) 항상 메인 페이지로 이동할 것.

- **`layout/navbar.html`**: "글 등록" `<a>`를 `sec:authorize="isAuthenticated()"` 블록 안으로
  옮겼다 — 이전에는 로그인 여부와 무관하게 항상 노출되던 유일한 버튼이었다. 비로그인 상태에서는
  더 이상 보이지 않는다(화면 자체는 여전히 `anyRequest().permitAll()`로 직접 URL 접근은
  가능 — 네비게이션 바 노출 여부만 바뀜, 인가 규칙 변경 아님).
- **`SecurityConfig.refererLogoutSuccessHandler()`**: 로그아웃 요청의 `Referer`가
  `/posts/save`(쿼리스트링 포함 여부 모두)일 때만 예외적으로 `/`로 보내고, 그 외 페이지는
  기존 로직(같은 오리진이면 그 페이지로, 아니면 `/`)을 그대로 따른다. 로그아웃하면 익명
  사용자가 되어 "글 등록" 화면 자체가 더는 의미가 없다는 게 이유다.

**검증**: 비로그인 상태에서 `/` 접속 → 네비게이션 바에 "글 등록" 버튼이 없음을 확인, 로그인 후
다시 표시됨을 확인. 로그인한 채 `/posts/save`로 이동해 Logout 클릭 → 확인 페이지 없이 즉시
로그아웃되고 **메인(`/`)으로 이동**함을 확인(다른 화면에서 로그아웃하면 그 페이지로 돌아가는
기존 동작과 대비됨).
