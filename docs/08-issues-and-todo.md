...
18. `UserDetailsService` 구현. 로그인 화면은 Spring Security 기본 로그인 페이지를 그대로 사용했다
    ...
    다른 화면과 동일한 디자인으로 통일했다. 이 변경의 부작용으로 `/logout`이 한때 404로 깨졌던
    것도 같은 날 발견해 함께 고쳤다(확인 페이지 없이 바로 로그아웃 + Referer 기반 리다이렉트).
    상세는 [06장](06-view-and-templates.md#로그인-화면-커스텀-템플릿-추가-2026-09-10),
...
## 8.16 [전체 기능 검증] 브라우저 + curl E2E 전수 검증 (2026-09-10)

P0~P3 구현이 모두 완료되고 91개 테스트가 통과하는 상태에서, 지금까지는 대부분 curl 기반으로만
검증해왔던 기능들을 **실제 Chrome 브라우저(Claude in Chrome 확장)를 직접 실행**해 사람이 쓰는
것과 동일한 방식으로 종합 재검증했다. 새로운 기능 구현이나 버그 수정은 없으며, 순수하게 기존
구현이 실제 브라우저 환경에서도 문제없이 동작하는지 확인하는 회귀성 점검이다.

### 브라우저로 직접 확인한 항목

`./gradlew bootRun` + `local` 프로파일로 기동한 뒤 Chrome에서 다음을 순서대로 수행했다.

1. 인덱스 화면(비로그인) → "Login"/"회원가입" 버튼 노출 확인
2. `/signup` 화면에서 실제 폼 입력으로 회원가입 → 서버 콘솔 로그(`ConsoleEmailSender`)에 찍힌
   인증 링크를 브라우저 주소창에 직접 입력해 이동 → "이메일 인증이 완료되었습니다" 성공 화면 확인
3. `/login`에서 실제 폼 로그인 → "Logged in as: ..." 헤더 표시 확인
4. `/posts/save`에서 제목·내용 입력 + **실제 파일 업로드**(`file_upload` 도구로 4×4 픽셀 PNG
   첨부) → 등록 후 게시글 목록에 반영, 상세(수정) 화면에서
   `document.querySelector('img').src`가 `/images/{uuid}.png`를 정확히 가리킴을 DOM에서 직접
   확인(단순 화면 캡처가 아니라 페이지 컨텍스트에서 JS로 값을 읽어 검증)
5. 게시글 제목 수정 → 목록에 반영 확인
6. 댓글 등록 → 작성자명·시각·내용이 화면에 정확히 표시됨을 확인
7. 비밀번호 변경 → 로그아웃(Spring Security 로그아웃 확인 화면 포함) → **옛 비밀번호로 로그인
   시도 시 "Invalid credentials" 거부** → **새 비밀번호로 로그인 성공**까지 실제 재로그인으로
   확인

### 브라우저 자동화 도구의 한계 (애플리케이션 결함 아님)

이 앱의 `static/js/app/index.js`는 게시글/댓글의 저장·수정·삭제 결과를 전부 네이티브
`alert()`로, 삭제 전 확인은 네이티브 `confirm()`으로 처리한다(구현 당시부터의 UX 설계이며 이번에
새로 발견된 것은 아님). Chrome DevTools Protocol(CDP) 기반 브라우저 자동화는 이 네이티브
다이얼로그에 입력을 전달할 방법이 없다:
- `alert()`는 페이지 이동(`navigate`)으로 강제 해제하면 그냥 닫히고 이후 로직(리다이렉트 등)이
  이어서 실행되어 우회 가능했다.
- 반면 `confirm()`은 페이지 이동으로 강제 해제하면 **항상 "취소"로 처리**되어(OK를 누른 것과
  동일한 효과를 낼 방법이 없음), 삭제 버튼 클릭 → `confirm()` → DELETE 요청으로 이어지는 흐름을
  브라우저 자동화만으로는 끝까지 실행할 수 없었다.

이는 CDP의 다이얼로그 처리 제약이지 애플리케이션의 결함이 아니다. 삭제(댓글·게시글) 흐름은
아래처럼 curl로 대체 검증해 실제로는 문제없이 동작함을 확인했다. 향후 이 앱을 브라우저
자동화로 재검증할 때는 삭제류 기능에서 같은 함정에 빠지지 않도록 이 사실을 기록해 둔다.

### curl로 보완 검증한 항목 (삭제 + 소유권 검증)

신규 계정 2개(`curltester2` — 작성자, `intruder` — 타인)를 curl로 가입·이메일 인증·로그인시킨
뒤 다음을 확인했다.

| 시나리오 | 결과 |
| --- | --- |
| 타인이 댓글 삭제 시도 (`DELETE /api/v1/comments/{id}`) | `403` `{"detail":"작성자 본인 또는 관리자만 수정·삭제할 수 있습니다. id=1", ...}` |
| 작성자 본인이 댓글 삭제 | `200`, 이후 `GET /api/v1/posts/{postId}/comments` 재조회 시 빈 배열 `[]` |
| 작성자 본인이 게시글 삭제 (`DELETE /api/v1/posts/{id}`) | `200` |
| 삭제된 게시글 재조회 (`GET /api/v1/posts/{id}`) | `400` `{"detail":"해당 게시글이 없습니다. id=2", ...}` |

### 결론

회원가입부터 이메일 인증, 로그인, 게시글/사진/댓글 CRUD, 비밀번호 변경, 소유권 검증, 삭제까지
전 기능이 실제 브라우저 렌더링과 curl 양쪽에서 정상 동작함을 재확인했다. 이번 검증으로 새로
발견된 버그는 없었다 — 순수 확인(regression check) 작업이었다.

## 8.17 [추가 구현] 로그인 후 네비게이션 정책 + 회원가입 검증 강화 + 이메일 암호화 (2026-09-10)

§8.16 검증 직후 사용자가 연속으로 요청한 4가지를 한 번에 구현했다.

### 네비게이션 바 정책 + 로그아웃 목적지 예외
- "글 등록" 버튼을 `sec:authorize="isAuthenticated()"` 블록 안으로 옮겨 로그인했을 때만 보이게
  했다(이전에는 로그인 여부와 무관하게 항상 노출).
- `SecurityConfig.refererLogoutSuccessHandler()`에 예외 규칙 추가: `/posts/save`에서 로그아웃하면
  (다른 화면과 달리) 항상 `/`로 보낸다. 상세는 [06장](06-view-and-templates.md#네비게이션-바-정책-조정--로그아웃-목적지-예외-2026-09-10) 참고.

### 회원가입 검증 강화
- 이름·이메일 DB 중복확인(`existsByName` 신규, 기존 이메일 중복확인은 해시 기반으로 변경 — 아래
  참고), 비밀번호 복잡도(대/소문자·특수문자, `@Pattern`), 비밀번호 2회 입력 확인(클라이언트
  전용, `signup.html`+`index.js`). 상세는 [04장 4.10절](04-architecture-and-layers.md#410-회원가입-검증-강화--이메일-암호화---신규-구현-2026-09-10) 참고.

### 이메일 암호화 (비밀번호는 이미 BCrypt)
- 처음에는 "SHA-512로 암호화"를 요청받았으나, 이메일은 인증 메일 재발송·화면 표시에 원문이
  필요해 단방향 해시만으로는 불가능함을 설명하고 AskUserQuestion으로 방식을 확인받았다 — 사용자가
  **"AES로 암호화된 `email` 컬럼 + SHA-512 해시로 조회하는 `email_hash` 컬럼 분리"** 방식을
  선택했다. 새 의존성 추가 없음(`spring-security-crypto`는 이미 전이 의존성). 상세 구현·검증은
  [03장 3.3절 7번 항목](03-domain-model.md#33-user), [04장 4.10절](04-architecture-and-layers.md#410-회원가입-검증-강화--이메일-암호화---신규-구현-2026-09-10),
  [07장 7.9절](07-configuration.md#79-이메일-암호화-키-설정-appsecurityemail-encryption-key---구현-완료-2026-09-10) 참고.

### 검증
`./gradlew test` → 기존 91개 + 신규 4개(이름 중복 400, 비밀번호 복잡도 400, `UserServiceTest`
이름 중복 예외, `existsByName` 리포지토리) = **95개 전부 통과**. `bootRun` + H2 콘솔로 `email`이
실제로 암호문(hex)으로, `email_hash`가 128자 SHA-512로 저장됨을 직접 확인했고, 그 상태에서도
로그인·회원가입 중복확인·네비게이션 바 노출·로그아웃 리다이렉트가 전부 정상 동작함을 브라우저로
재검증했다.
