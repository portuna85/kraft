...
| [09. 구현 요약](09-implementation-summary.md) | P0+P1 구현 완료 내용과 실제 검증(curl E2E) 결과 |

## 현재 상태 요약

**P0(빌드 복구) + P1(실사용 가능한 상태) 구현이 완료되었습니다.** `./gradlew compileJava`, `./gradlew test`가
모두 통과하며, 회원가입 → 로그인 → 게시글 등록/조회/수정/삭제까지 브라우저·curl로 실제 검증했습니다.
상세 변경 내역과 검증 결과는 [09. 구현 요약](09-implementation-summary.md)을 참고하세요.

`01~08` 문서는 구현 이전 분석을 기준으로 작성되었으며, 이번 구현으로 해결된 항목은 각 문서에 해결 표시를
추가했습니다. 남아 있는 문제(P2/P3 — 전역 예외 처리, 작성자 권한 검증, 페이징, 댓글 기능 등)는
[08. 이슈와 TODO](08-issues-and-todo.md)에 그대로 유효합니다.
