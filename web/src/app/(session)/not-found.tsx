 ...
 * 반환한다. `(session)/**` 하위에 이 파일이 없으면, 매치된 라우트 안에서
 * `notFound()`가 호출될 때(예: `community/posts/[id]/page.tsx`) 이미 마운트된
 * `(session)/layout.tsx`의 `<main id="main">` 안에 루트 not-found가 다시
 * 렌더돼 main 랜드마크가 중첩되고 id가 중복됐다. 세션 레이아웃이 이미 main을
