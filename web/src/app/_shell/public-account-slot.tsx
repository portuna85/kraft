 ...
 * `CommunityLoginHandler`가 로그인 성공 시 심는다)만 먼저 읽어 판단한다. 로그인
 * 사용자에 한해서만 `PublicAccountMenu`(세션 API 호출 + `(session)` 셸과 동일한
 * 닉네임/로그아웃 메뉴)를 마운트한다 — 그래서 페이지 그룹과 무관하게 로그인 사용자는
 * 항상 같은 계정 메뉴를 본다. 예전에는 이 자리가 항상 `LoginPopover`였다 — OAuth
 * 성공이 항상 `/`로 착지하는데 `/`가 세션을 못 읽어 로그인 성공과 로그아웃 상태가
 * 픽셀 단위로 같았다.
 */
