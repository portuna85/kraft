  ...
  privacy: {
    content: (
      <article className={styles.article}>
        ...
        <h2>보관 기간</h2>
        <p>
          저장 번호와 추천 이력은 사용자가 직접 삭제할 때까지 유지됩니다. <strong>탈퇴만으로는 삭제되지 않으며</strong>,
          탈퇴 전에 보관함에서 직접 지우거나 탈퇴 후 문의하기 페이지로 삭제를 요청해야 합니다. 게시글·댓글·신고·운영
          기록의 보관 기간과 광고 관련 처리의 세부 사항은 법률 검토를 거쳐 별도 개정 고지로 확정합니다.
        ...
        <p>
          탈퇴 시 처리는 다음과 같습니다. 계정은 삭제되지 않고 <strong>탈퇴 상태로 전환</strong>되며, 닉네임은
          &ldquo;탈퇴한 사용자&rdquo; 형태로 가명 처리되고 프로필 이미지는 삭제됩니다. 기존에 작성한 게시글·댓글은
          그대로 남되 작성자 표시가 같은 가명으로 바뀝니다. 로그인에 사용한 소셜 계정 식별자는 재로그인 확인을 위해
          <strong>계속 보관</strong>되며, 같은 소셜 계정으로 다시 로그인하면 <strong>같은 계정이 다시 활성화</strong>됩니다.
          저장 번호·추천 이력 등 계정에 연결된 데이터도 탈퇴로 삭제되지 않습니다.
        </p>
        <p>
          계정과 연결된 데이터의 완전한 삭제, 게시물 처리, 추가 열람·정정 요청은 문의하기 페이지를 통해 접수할 수 있습니다.
        </p>
