...
const infoPages: Record<InfoPageSlug, InfoPage> = {
  ...
  methodology: {
    content: (
      <article className={styles.article}>
        ...
        <p>
          <strong>균형 조합 추천</strong>은 홀짝 비율, 고저 비율, 번호 합계, 연속 번호,
          구간 분포가 한쪽으로 치우치지 않은 조합을 우선 선택합니다. 홀수·짝수 개수가
          극단(0개 또는 6개)에 몰리지 않고, 저번호(1~22)·고번호(23~45) 비율과 번호 합계가
          중앙 구간에 가까우며, 번호가 여러 구간에 고르게 퍼진 조합일수록 우선순위가 높습니다.
          균형 조합 역시 당첨 확률을 높이지 않습니다 — 번호를 고르는 하나의 방식일 뿐이며,
          치우친 조합이 당첨될 확률과 균형 잡힌 조합이 당첨될 확률은 동일합니다.
        </p>
        <p>
        ...
        <h2>세후 당첨금 계산</h2>
        <p>
          당첨금 200만원 이하는 비과세이며, 200만원 초과 3억원 이하 구간에는 22%(기타소득세 20% + 지방소득세 2%),
          3억원 초과 구간에는 33%(기타소득세 30% + 지방소득세 3%)의 세율을 구간별로 적용해 합산합니다.
          예를 들어 1등 당첨금이 3억원을 넘으면 3억원까지는 22% 세율을, 3억원을 넘는 나머지 금액에는 33% 세율을 각각 적용합니다.
          세율 근거는 소득세법 제129조 및 복권 및 복권기금법 시행령 제8조의2입니다.
        </p>
  ...
  privacy: {
    content: (
      <article className={styles.article}>
        ...
        <h2>수집하는 정보</h2>
        <p>
          KRAFT Lotto의 당첨 조회·추천 기능은 회원가입 없이 이용할 수 있습니다. 보관함 기능을 사용할 때는
          브라우저에 익명 기기 토큰(UUID)이 생성되며, 이 토큰과 연결된 보관함과 추천 이력이 서버에 보관될 수 있습니다.
        ...
        <h2>보관 기간</h2>
        <p>
          보관함과 추천 이력은 사용자가 직접 삭제하거나 계정 탈퇴 시 삭제됩니다. 탈퇴 시 OAuth 식별자, 프로필,
          기기 귀속, 커뮤니티 반응·신고·차단 기록도 함께 삭제됩니다. 게시글·댓글은 다른 이용자의 대화 맥락을 보호하기 위해
          내용과 작성자 정보를 제거한 삭제 표시로만 남습니다. 광고 관련 처리의 세부 사항은 법률 검토를 거쳐 별도 개정 고지로 확정합니다.
        ...
        <p>
          ...
          &ldquo;탈퇴&rdquo; 버튼으로 직접 탈퇴할 수 있습니다.
        </p>
        <p>
          탈퇴 시 계정과 OAuth 식별자, 프로필 이미지, 보관함, 추천 이력, 기기 귀속 및 커뮤니티 상호작용 기록을
          <strong>영구 삭제</strong>합니다. 기존 게시글·댓글은 다른 이용자의 답글과 반응을 보호하기 위해 작성자 연결과
          본문을 제거한 삭제 표시로만 남습니다. 같은 소셜 계정으로 다시 로그인하면 이전 데이터를 복구하지 않는 새 계정이 생성됩니다.
        </p>
        <p>
          개인정보 관련 열람·정정 요청은 문의하기 페이지를 통해 접수할 수 있습니다.
        </p>
  ...
  terms: {
    content: (
      <article className={styles.article}>
        ...
        <h2>제2조 서비스 내용</h2>
        <p>
          서비스는 동행복권 공식 데이터를 기반으로 과거 당첨 결과 조회, 통계 분석,
          번호 추천 기능을 제공합니다. 번호 추천은 무작위, 균형 조합, 공동 당첨 위험 완화
          전략을 제공하며, 각 전략은 번호를 고르는 방식이 다를 뿐입니다.
          모든 유효한 조합의 1등 당첨 확률은 동일하며, 서비스의 어떤 기능도
          당첨을 예측하거나 보장하지 않습니다.
        </p>
    ...
    content: (
      <article className={styles.article}>
        ...
        <p>
          ...
          운영하는 서비스입니다. 서버 운영비를 충당하기 위해 광고를 게재하며,
          그 밖의 영리 목적은 두지 않습니다. 번호를 판매하거나 유료 기능을 제공하지 않습니다.
        </p>
  ...
  "community-guidelines": {
    content: (
      <article className={styles.article}>
        <p>
          KRAFT Lotto 커뮤니티는 번호 추천과 당첨 정보를 나누는 공간입니다.
          모두가 안전하게 이용할 수 있도록 아래 규칙을 지켜 주세요.
        </p>
        ...
        <h2>신고와 제재</h2>
        <p>
          로그인한 뒤 게시글 하단과 각 댓글 옆의 신고 버튼으로 문제가 되는 콘텐츠를 신고할 수 있습니다.
          본인이 작성한 글과 댓글에는 신고 버튼이 표시되지 않습니다. 특정 이용자가 반복해서 문제를 일으키는
          경우에는 해당 사용자를 차단하거나 문의하기 페이지로 알려 주세요.
          접수된 신고는 운영진이 검토하며, 규칙을 위반한 게시물·댓글은 삭제되거나
          작성자의 게시·댓글 작성이 제한될 수 있습니다.
        </p>

        <h2>콘텐츠 책임</h2>
        <p>
          게시글과 댓글의 내용에 대한 책임은 작성자 본인에게 있으며,
          KRAFT Lotto가 게시된 정보의 정확성을 보증하지 않습니다.
        </p>
      </article>
    ),
  },
