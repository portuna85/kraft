...
export default defineConfig({
  test: {
    ...
    coverage: {
      ...
      include: ["src/**/*.{ts,tsx}"],
      // src/app/**(api 제외)는 Next.js 페이지·레이아웃 등 RSC/스트리밍 구성 코드로,
      // 실제 Next 런타임 없이는 vitest+jsdom으로 의미 있게 단위 테스트하기 어렵다 —
      // e2e(Playwright)가 담당하는 영역이라 커버리지 분모에서 제외한다. 반면
      // src/app/api/**의 라우트 핸들러는 평범한 Node 함수라 vitest로 바로 테스트
      // 가능하고 실제로 전부 테스트가 있다(T-3) — 분모 제외 대상에서 뺀다.
      exclude: [
        "**/node_modules/**",
        "**/e2e/**",
        "**/*.config.*",
